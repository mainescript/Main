...
local kWin, kFrame = library:AddWindow("💎 SCP HUB PREMIUM — Key System", {
    main_color          = Color3.fromRGB(180,180,180),  -- Plateado principal
    title_bar           = {Color3.fromRGB(200,200,200), Color3.fromRGB(80,80,80)},  -- Plateado claro y gris oscuro
    background          = {Color3.fromRGB(18,18,20)},  -- Negro azulado
...
kt:AddTextBox("Paste key here then press Enter...", function(v)
    if checkKey(v) then
        stLbl.Text = "✅  Key accepted!  Loading SCP HUB PREMIUM..."
        keyPassed = true
    else
...
pcall(function()
    ...
    if bar then local tog=bar:FindFirstChild("Toggle")
        if tog then tog.Image="rbxassetid://6031075931"; tog.ImageColor3=Color3.fromRGB(220,220,220) end
    end
end)
-- ══════════════════════════════════════════
--  TAB: INFO
...
-- Show info tab first
infoTab:Show()
notify("SCP HUB PREMIUM","💎 V5.0 Loaded! RShift to toggle  💎  Made by TEJAZ",5)
